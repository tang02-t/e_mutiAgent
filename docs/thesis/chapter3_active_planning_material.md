...
> 对应计划 B-6；数字来自 `docs/eval/attribution_calibration.md`（B-1）、`docs/eval/eig_sanity_check.md`（B-2）、`data/eval/d12/DATA_CARD.md`（B-3）、`docs/eval/active_planning_eval.md`（B-5）。代码锚点：`src/tools/fault_attribution.py`、`src/tools/eig.py`、`src/agents/planner.py`、`src/graph/workflow.py`、`scripts/sim/partial_obs_sim.py`、`scripts/eval/eval_active_planning.py`。
...
| 表 3-1 | 本文第 1 节 | 符号表 |
| 表 3-2 | `docs/eval/eig_sanity_check.md` | 20 条手工样例推荐一致率 90% / 95%，κ 敏感性 |
| 表 3-3 | 本文第 5 节 | 协议 B 主结果 |
| 表 3-4 | `docs/eval/active_planning_eval.md` §3 | 配对检验 |
| 表 3-5 | `docs/eval/active_planning_eval.md` §5 | 校准 / 成本 / τ_H 消融 |
