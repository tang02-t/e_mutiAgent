...
|---|---|---:|---|---|---|---|
| R1 | `data/dga/dga_dataset.csv` | 4150 行；分号分隔、逗号小数 | 项目说明标注为 Kaggle，未确认到具体发布页 | 未核实 | 执行环境（学习 CPT）、评测标签（气体→IEC 故障性质） | 直接作为工具调用监督 |
| R2 | `data/dga/data.xlsx` | 2321 行；Sheet1 | `alan-456/transformer-fault-dataset`，仓库说明为数据库 + 论文 + 部分中国电网数据汇集 | 仓库未声明许可 | 同 R1 | 同 R1 |
| R3 | `data/dga/dataset_(589).xlsx` | 589 行 | 同 R2 仓库，文献汇集 | 同 R2 | 同 R1 | 同 R1 |
...
| R6 | `data/ETT-small/ETTh1.csv`、`ETTh2.csv` | 各 17420 行，小时级 | ETDataset（Informer, AAAI 2021），国网富达两站 2016-07 至 2018-06 | CC BY-ND 4.0（原仓库） | 执行环境（油温预测）、评测标签（未来真实 OT）、任务种子 | 不能与 DGA 拼成同一设备 |
| R7 | `data/ETT-small/ETTm1.csv`、`ETTm2.csv` | 各 69680 行，15 分钟级 | 同 R6 | 同 R6 | 同 R6 | 同 R6；ETTh 与 ETTm 疑似同源序列，不能视为独立设备做泛化测试 |
| R8 | `data/external_transformer/power_transformer_fault.csv` | 1866 行，106 台变压器，含 9 种气体、油质、糠醛及 4 种经典比值法诊断列 | 来源未记录，需补充 | 未核实 | 候选：评测标签（Duval / Rogers 等诊断列作为规则法参考）、任务种子（含设备编号与多次测量，可构造趋势类任务） | 未核实来源前不入训练集 |
...
- 三份来源之间存在重复/近重复记录，切分前需按气体向量聚类去重。
...
- 来源、许可未记录；`Year` 有空值；四种诊断列大量为 Undefined/Unidentifiable。
- 优点：有真实设备编号与多次测量号，是目前唯一能构造"同一设备趋势"任务的真实数据；需先补充来源再决定是否使用。
