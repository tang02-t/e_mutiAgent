...
> C-1 产出。Generator 在 `workflow.generator_output_mode = claims` 下先输出 JSON 声明列表再渲染自然语言草案；LLM 不可用或 JSON 非法时由 [claims.py](/Users/ts/Desktop/thu/multi_Agent/src/agents/claims.py) 的 `build_rule_claims` 按工具结果规则拼装，链路不断。声明存于 `AgentState.draft_claims`，供 C-2 约束核查器逐条核对。
