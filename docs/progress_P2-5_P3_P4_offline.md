...
- 接入：`run_diagnosis_workflow(..., reflector=...)` 在 `retriever → reflection → generator` 插入节点；`app.py` 侧栏"反思模块"开关；`scripts/eval_end2end.py --reflection {off,lexical,llm}`，报告新增反思决策分布、保留率、改写触发数。
