...
- 预测文件：`data/planner/predictions/M0_qwen3.7-flash_s42.jsonl`（`.gitignore` 排除，260 KB，本机保留）；报告 `docs/planner_eval.md`。
...
- 命令：`eval_system_modes.py --modes mode2 --limit 50 --tag a1_baseline`（旁路结果 `data/eval/d10/results/mode2_a1_baseline.jsonl`，不进 `docs/end2end_eval.md`）。
