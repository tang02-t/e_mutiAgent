...
每个变体 2 个 seed（42 / 2026），`run_matrix.sh` 一键完成训练 → 推理 → 评测；报告 `docs/eval/planner_eval.md` 对同变体多 seed 取均值。Q0 / Q1（查询分解智能体有无微调）在 v2 中不做。
...
6. 本地 `config.yaml` 填 `llms.planner_finetuned`（provider dashscope，model_name 为部署 ID），`python3 scripts/dev/probe_llm_endpoint.py`
7. `python3 scripts/eval/eval_end2end.py --limit 3 --planner-mode finetuned` 确认能跑通
