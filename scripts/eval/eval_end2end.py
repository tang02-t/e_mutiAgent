...
用法：
  python3 scripts/eval/eval_end2end.py --limit 20                   # 开发回归，先跑 20 条
  python3 scripts/eval/eval_end2end.py --kb-mode mock               # 用合成案例 mock 替代本地知识库（仅流程调试）
  python3 scripts/eval/eval_end2end.py --no-llm                     # 强制不调用 LLM（仅评测流程+规则）
  python3 scripts/eval/eval_end2end.py --eval data/real/eval/xxx.jsonl --purpose eval_set   # 正式评测
"""
