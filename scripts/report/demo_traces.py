...
三条主线的「从输入到答案」逐步执行轨迹（离线、零 LLM、零费用），供快速理解系统如何运转。
  ...
  python3 scripts/report/demo_traces.py                 # 打印三条轨迹
  python3 scripts/report/demo_traces.py --write         # 同时写入 docs/eval/walkthrough_traces.md
...
OUT_MD = ROOT / "docs/eval/walkthrough_traces.md"
LINES: List[str] = []
...
def main() -> None:
    ...
    ap.add_argument("--write", action="store_true", help="写入 docs/eval/walkthrough_traces.md")
    ...
    p()
    p("> 由 `python3 scripts/report/demo_traces.py --write` 生成。每段轨迹都走与正式实验相同的代码路径，")
