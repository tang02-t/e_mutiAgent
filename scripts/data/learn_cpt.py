...
v2（B-1）新增：
  ...
  - 可靠性图 docs/figures/fig_b1_reliability.png，报告 docs/eval/attribution_calibration.md；
  - 最终用全量数据重学 prior / CPT，并把 5 折平均的 T 与 w_f 写入 learned_params.json 的
    `calibration` / `fusion_weights` 字段，供 FaultBayesianNetwork 加载。

评测口径：只评估非 normal 样本（引擎不输出 normal），与 scripts/eval/eval_fault_attribution.py 一致；
negative evidence（低于注意值的气体征兆记为 False）默认开启，可用 FAULT_ATTR_NEG_EVIDENCE=0 关闭做消融。

用法：
  python3 scripts/data/learn_cpt.py                       # 留出法 70/30（v1 行为）
  python3 scripts/data/learn_cpt.py --kfold 5             # 5 折交叉验证 + 校准 + 报告（B-1）
  python3 scripts/data/learn_cpt.py --kfold 5 --subset "dataset_(589).xlsx"   # 只看 589 公开基准子集
"""
...
def write_report(cv: dict[str, Any], out_md: Path, fig_rel: Path | None, subset_tag: str, neg_ev: bool,
    ...
    lines = [
        "# 故障归因引擎校准报告（B-1）",
        "",
        f"> 生成：`python3 scripts/data/learn_cpt.py --kfold {cv['k']} --seed {cv['seed']}`"
    ...
    lines += [
        ...
        "## 6. 说明与局限",
        "",
        "- 数据为文献汇编 DGA（见 `docs/design/data_and_evaluation.md` §1.1），标签仅有 4 个故障类（局部放电 / 匝间短路 / 过载过热 / 绝缘老化）+ normal；"
        ...
        "但直接影响 B-2 EIG 对现场征兆的推荐强度，其敏感性见 `docs/eval/eig_sanity_check.md`。",
...
def main():
    ...
    ap.add_argument("--report", default=str(ROOT / "docs/eval/attribution_calibration.md"))
