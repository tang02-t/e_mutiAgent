# Planner SFT 数据卡片（含 A-2 LLM 口语化改写）

- 来源：`data/planner/seeds/task_seeds_rewritten.jsonl`（P4 任务种子，金标动作已参数校验 + 真实执行；其中 4538 条为 `rewrite_queries.py` 口语化改写，`seed_id` 带 `-rN` 后缀、`rewrite_of` 指向原种子，沿用原种子 `gold_actions` / `group_key` / `split`；test 切分不改写）；多轮/错误恢复：`data/planner/seeds/multi_turn_seeds.jsonl`（工具返回全部来自真实执行）
...
- 已知偏差：train / dev 问法为模板 + LLM 口语化改写（qwen3.7-flash，数字 / 实体 / 判断词守卫），test 仍为模板问法；数值类 DGA 记录来自 3 个公开数据集，标签分布不均（过载过热/正常偏多）；图谱推理类受规则抽取图谱覆盖限制（90 节点/190 边）；多轮样本中 assistant 的 thought 为规则模板文本
- 错误恢复样本：刻意错误的首次调用只保留在上下文中，不导出为训练目标（decision_index 从 1 起）；tool 返回为真实执行结果
- 许可：文献数据仅用于内部研究；ETT 数据集 CC BY-ND 4.0；DGA 数据集见 docs/design/data_and_evaluation.md §1.1
...
| train | composite | 461 |
| train | fact | 2280 |
| train | insufficient | 762 |
| train | no_tool | 548 |
| train | numeric_tool | 1643 |
| train | reasoning | 437 |
